...
export const fetchBlog = createAsyncThunk("blog/fetchBlog", async()=>{
    return fetch(`http://3.110.118.82:1337/api/blogs?sort=[date]&&populate=*`).then(
    (res)=> res.json()
    )
});
export const fetchBlogSearch= createAsyncThunk("blog/fetchBlog/search", async({value})=>{
console.log({value});
    return fetch(`http://3.110.118.82:1337/api/blogs?sort=[date]&&populate=*&&filters[$or][0][title][$contains]=${value}&filters[$or][1][description][$contains]=${value}&filters[$or][2][blog_category][category_name][$contains]=${value}`).then(
    (res)=> res.json()
    );
});
...
export const fetchBlogDetails = createAsyncThunk("blog/fetchBlogDetails", async({slug})=>{
    return fetch(`http://3.110.118.82:1337/api/blog-details?populate=*&&filters[slug][$eq]=${slug}`).then(
    (res)=> res.json()
    );
});
