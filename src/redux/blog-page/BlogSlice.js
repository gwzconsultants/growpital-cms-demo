...
export const fetchBlog = createAsyncThunk("blog/fetchBlog", async()=>{
    return fetch(`${process.env.REACT_APP_BASE_URL}/api/blogs?sort=[date]&&populate=*`).then(
    (res)=> res.json()
    )
});
export const fetchBlogSearch= createAsyncThunk("blog/fetchBlog/search", async({value})=>{
console.log({value});
    return fetch(`${process.env.REACT_APP_BASE_URL}/api/blogs?sort=[date]&&populate=*&&filters[$or][0][title][$contains]=${value}&filters[$or][1][description][$contains]=${value}&filters[$or][2][blog_category][category_name][$contains]=${value}`).then(
    (res)=> res.json()
    );
});
